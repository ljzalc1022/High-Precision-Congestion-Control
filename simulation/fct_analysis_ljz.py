...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print(f"usage: {sys.argv[0]} TcpType baseline magic")
        exit()
    TcpType = sys.argv[1]
    baseline = sys.argv[2]
    magic = sys.argv[3]

    data = pd.read_csv(os.sep.join(["data", "FT", TcpType, baseline + "-fct.dat"]),
                       header=None,
                       names=["flow_size", "FCT"],
                       skiprows=[0],
    ...
    data = pd.read_csv(os.sep.join(["data", "FT", TcpType, magic + "-fct.dat"]),
                       header=None,
                       names=["flow_size", "FCT"],
                       skiprows=[0],
                       delim_whitespace=True)
    ...
    legend = plt.legend(handles=[bar_1, bar_2, bar_3], loc="right",
                        ...
                        columnspacing=0.5, ncol=6, bbox_to_anchor=(.68, 0.88, 0.2, 0.4))

    plt.ylim(0, 1.1)
    plt.yticks([0, 0.25, 0.5, 0.75, 1], ["0", "", "0.5", "", "1.0"])

    # plt.xlim()
    plt.xticks(index, ["Average", "95-pct", "99-pct"])
    # plt.xticks(index, ["Average", "50-pct", "95-pct", "99-pct"])
    plt.tick_params(labelsize=18)

    plt.grid(True, axis="y", linestyle='--', alpha=0.5, linewidth=2)
    ax.set_ylabel("Relative FCT", fontsize=20)
    ax.set_axisbelow(True)

    fig.tight_layout()
    plt.subplots_adjust(left=0.15, bottom=0.13, right=0.99, top=0.87)
    plt.show()
    plt.savefig(f"{TcpType}-{magic}")
