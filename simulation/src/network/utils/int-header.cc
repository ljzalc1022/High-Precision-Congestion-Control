...
void IntHeader::PushHop(uint64_t time, uint64_t bytes, uint32_t qlen, uint64_t rate, 
						bool isBigflow, uint64_t heavyBytes, uint64_t targetRate, uint32_t card, 
						uint16_t p) {
	...
	if (mode == NORMAL){
		...
		hop[idx].SetMyCC(targetRate, heavyBytes, card);
		nhop++;
...
void IntHeader::Serialize (Buffer::Iterator start) const{
	...
	if (mode == NORMAL){
		for (uint32_t j = 0; j < maxHop; j++){
			...
			i.WriteU32(hop[j].myBuf[0]);
			i.WriteU32(hop[j].myBuf[1]);
		}
		i.WriteU16(nhop);
...
uint32_t IntHeader::Deserialize (Buffer::Iterator start){
	...
	if (mode == NORMAL){
		for (uint32_t j = 0; j < maxHop; j++){
			...
			hop[j].myBuf[0] = i.ReadU32();
			hop[j].myBuf[1] = i.ReadU32();
		}
		nhop = i.ReadU16();
