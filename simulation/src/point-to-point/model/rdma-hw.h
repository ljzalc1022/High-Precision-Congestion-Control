...
public:
	...
	void AddQueuePair(uint64_t size, uint16_t pg, Ipv4Address _sip, Ipv4Address _dip, uint16_t _sport, uint16_t _dport, uint32_t win, uint64_t baseRtt, double endTime, Callback<void> notifyAppFinish); // add a new qp (new send)
