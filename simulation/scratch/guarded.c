...
int rx_id_min = 5, rx_id_max = 9, rx_num = 5;
...
// uint64_t rxBytes[5];
std::vector<std::ofstream> rxThru;
...
Time measurementInterval = NanoSeconds(60720);
...
int main(int argc, char *argv[])
{
...
#endif
	{
		...
		while (!conf.eof())
		{
			...
			}else if (key.compare("RX_ID_MIN") == 0){
                conf >> rx_id_min;
                std::cout << "RX_ID_MIN\t\t\t\t" << rx_id_min << '\n'; 
            }else if (key.compare("RX_ID_MAX") == 0){
                conf >> rx_id_max;
                std::cout << "RX_ID_MAX\t\t\t\t" << rx_id_max << '\n';
            }else if (key.compare("RX_NUM") == 0){
                conf >> rx_num;
                std::cout << "RX_NUM\t\t\t\t" << rx_num << '\n';
    ...
    NS_ASSERT(rx_num == rx_id_max - rx_id_min + 1);
    rxBytes.reserve(rx_num);
    rxThru.reserve(rx_num);
	...
	for (uint32_t i = 0; i < node_num; i++){
		if (n.Get(i)->GetNodeType() == 0){ // is server
			...
			if (i >= rx_id_min && i <= rx_id_max)
				rdmaHw->TraceConnectWithoutContext("Rx", MakeBoundCallback(TraceSinkRx, i - rx_id_min));
