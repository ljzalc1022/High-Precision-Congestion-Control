...
void TraceSinkTx(std::size_t index, Ptr<Packet> p, Ptr<RdmaQueuePair> qp)
{
	txBytes[index] += p->GetSize();
}

void TraceSinkRx(std::size_t index, Ptr<Packet> p, Ptr<RdmaRxQueuePair> qp)
{
	rxBytes[index] += p->GetSize();
}

void TraceRate(std::size_t index, uint64_t old_rate, uint64_t new_rate)
{
	double time = Simulator::Now().GetSeconds();

	if (time >= trace_rate_start && time <= trace_rate_end)
		rate_output << Simulator::Now().GetNanoSeconds() << " " << old_rate << " " << new_rate << std::endl;
}

Time measurementInterval = MicroSeconds(10);
void PrintThroughput(Time measurementTime)
{
	for (std::size_t i = 0; i < 3; ++i)
	{
		Thpt[i] << std::fixed << measurementTime.GetSeconds() << " "
				  << (txBytes[i] * 8) / measurementInterval.GetSeconds() / 1e9 << std::endl;
		txBytes[i] = 0;
	}
	for (std::size_t i = 0; i < 3; ++i)
	{
		Gdpt[i] << std::fixed << measurementTime.GetSeconds() << " "
...
int main(int argc, char *argv[])
{
...
#endif
	{
		...
		std::cout << "Running microburst.cc with config: \'" << argv[1] << "\' at " << asctime(localTime) << std::endl;
#else
		...
		while (!conf.eof())
		{
            ...
            }else if (key.compare("MAGIC_MB_MODE") == 0){
                conf >> magic_mb_mode;
                std::cout << "MAGIC_MB_MODE\t\t\t\t" << magic_mb_mode << '\n';
	...
	std::string rate_output_file = output_folder + "rate_long.dat";
	rate_output.open(rate_output_file, std::ofstream::out);
    ...
    Config::SetDefault("ns3::CmSketch::MicroburstMode", EnumValue(magic_mb_mode));
	
	// Config::SetDefault("ns3::RdmaHw::TraceRate", BooleanValue(trace_rate));
	...
	for (uint32_t i = 0; i < node_num; i++){
		if (n.Get(i)->GetNodeType() == 0){ // is server
			...
			if (i >= 3 && i <= 5)
				rdmaHw->TraceConnectWithoutContext("Rx", MakeBoundCallback(TraceSinkRx, i - 3));
	...
	for (std::size_t i = 0; i < 3; ++i)
	{
		std::string thpt_file = output_folder + "thpt_flow_" + std::to_string(i) + ".dat";
        std::string gdpt_file = output_folder + "gdpt_flow_" + std::to_string(i) + ".dat";
		Thpt[i].open(thpt_file, std::ios::out);
