...
void monitor_qlen(NodeContainer *n){
	Ptr<SwitchNode> sw = DynamicCast<SwitchNode>(n->Get(10));
	uint32_t size = 0;
	for (uint32_t k = 0; k < SwitchMmu::qCnt; k++)
		size += sw->m_mmu->egress_bytes[6][k];
	qLen << Simulator::Now().GetNanoSeconds() << " " << size << std::endl;
...
Time measurementInterval = NanoSeconds(2000);
...
int main(int argc, char *argv[])
{
	...
	for (uint32_t i = 0; i < node_num; i++){
		if (n.Get(i)->GetNodeType() == 0){ // is server
			...
			// if (i == 4)
			// 	rdmaHw->SetAttribute("TraceRate", BooleanValue(true));
	...
	qLen.open("./experiment-results/hpcc/microburst/magic/magic_5_100/qLen.dat");
	...
	std::string output_folder = "./experiment-results/hpcc/microburst/magic/magic_5_100/";
	for (std::size_t i = 0; i < 5; ++i)
	{
